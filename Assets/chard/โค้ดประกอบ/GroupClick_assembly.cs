...
public class GroupClick_assembly : MonoBehaviour
{
    public int groupNumber; // กำหนดตัวเลขกลุ่ม 1, 2 หรือ 3
    public GameObject[] groupObjects; // ออปเจคในกลุ่มนี้
    public float outlineWidth = 6.0f; // ขนาดของ Outline Width ที่จะกำหนด
...
if (!EventSystem.current.IsPointerOverGameObject())
            {
                ...
                CentralScript_assembly.Instance.ReceiveGroupNumber(groupNumber, groupObjects);
    ...
    {
        int currentStep = CentralScript_assembly.Instance.GetCurrentStep();
        int ToolsNumber = ToggleButtonsMultiple_assembly.Instance.GetToolsNumber();
        int stack1 = CentralScript_assembly.Instance.GetStack1();
        int stack2 = CentralScript_assembly.Instance.GetStack2();
        int stack3 = CentralScript_assembly.Instance.GetStack3();
        ...
        foreach (GameObject obj in groupObjects)
        {
            ...
            HashSet<int> CounterBalanceALL = new HashSet<int> { 112 , 117 };
            ...
            HashSet<int> CONNECTER_MOTOR3 = new HashSet<int> { 11, 12 };
                if (stack1 >= 36 && stack2 >= 19 && stack3 >= 23){
                    if (currentStep == groupNumber)
                    {
                        ...
                        else if(AXIS456.Contains(groupNumber) && ToolsNumber == 0){outline.OutlineColor = Color.green;}
                        else if(CounterBalance1.Contains(groupNumber) && ToolsNumber == 2){outline.OutlineColor = Color.green;}
                        else if(CounterBalance2.Contains(groupNumber) && ToolsNumber == 0){outline.OutlineColor = Color.green;}
                        else if(CounterBalance3.Contains(groupNumber) && ToolsNumber == 0){outline.OutlineColor = Color.green;}
                        else if(CounterBalance4.Contains(groupNumber) && ToolsNumber == 9){outline.OutlineColor = Color.green;}
                        else if(AXIS2.Contains(groupNumber) && ToolsNumber == 0){outline.OutlineColor = Color.green;}
                        else if(CounterBalanceALL.Contains(groupNumber) && ToolsNumber == 0){outline.OutlineColor = Color.green;}
                        else
                        {
